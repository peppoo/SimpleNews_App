...
export const getStaticProps = async () => {
  ...
  const topRes = await fetch(
    `https://newsapi.org/v2/top-headlines?country=jp&pageSize=${pageSize}&apiKey=9f16326e2a0d4dc8a186768e2b4cf766`
  );
  ...
  const weatherRes = await fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=metric&exclude=${exclude}&appid=06b7e8741de1dab01f741b1cf85f4eeb`
  );
  ...
  const pickupRes = await fetch(
    `https://newsapi.org/v2/everything?q=${keyword}&language=jp&sortBy=${sortBy}&pageSize=${pickupPageSize}&apiKey=9f16326e2a0d4dc8a186768e2b4cf766`
  );
